...
void CreateDigest (IMemoryBlock &Data, DIGEST *retDigest);
void CreateDigest (const CString &sData, DIGEST *retDigest);
...
void CCryptoDigest::SHAInit(SHA_CTX *shsInfo)
{
...
void CCryptoDigest::SHAUpdate(SHA_CTX *shsInfo, BYTE *buffer, int count)
{
    UINT4 tmp;
    int dataCount;
...
void CCryptoDigest::SHAFinal(BYTE *output, SHA_CTX *shsInfo)
{
    int count;
    BYTE *dataPtr;
...
//	See: http://tools.ietf.org/html/rfc2104
	...
	if (KeyBytes.GetLength() > SHS_DATASIZE)
		{
		CCryptoDigest SHA1;
		SHA1.AddData(KeyBytes);
		SHA1.CalcDigest(MACKey);
		...
		{
		BYTE *pSrc = (BYTE *)KeyBytes.GetPointer();
		BYTE *pSrcEnd = pSrc + KeyBytes.GetLength();
		BYTE *pDest = MACKey;
	...
	CCryptoDigest SHA1;
	SHA1.AddData(InnerPad, SHS_DATASIZE);
	SHA1.AddData(Data);
	SHA1.CalcDigest(InnerDigest);
	...
	SHA1.Reset();
	SHA1.AddData(OuterPad, SHS_DATASIZE);
	SHA1.AddData(InnerDigest, SHS_DIGESTSIZE);
	SHA1.CalcDigest(MAC);

	//	Return

	retMAC->InitFromBytes(CBuffer(MAC, sizeof(MAC), false));
	}
