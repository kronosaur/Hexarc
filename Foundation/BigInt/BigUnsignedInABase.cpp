...
BigUnsignedInABase::operator CString () const

//	BigUnsignedInABase operator CString
	...
	CString sString((LPSTR)NULL, len);
	...
	for (symbolNumInString = 0; symbolNumInString < len; symbolNumInString++) 
		...
		Digit theDigit = blk[digitNum];
		if (theDigit < 10)
			pPos[symbolNumInString] = char('0' + theDigit);
		else
			pPos[symbolNumInString] = char('A' + theDigit - 10);
		}

	return sString;
	}

void BigUnsignedInABase::WriteToStream (IByteStream &Stream)
	{
	if (base > 36)
		throw "BigUnsignedInABase ==> std::string: The default string conversion routines use the symbol set 0-9, A-Z and therefore support only up to base 36.  You tried a conversion with a base over 36; write your own string conversion routine.";
	if (len == 0)
	...
	for (symbolNumInString = 0; symbolNumInString < len; symbolNumInString++) 
		...
		char chChar;
		if (theDigit < 10)
			chChar = char('0' + theDigit);
		else
			chChar = char('A' + theDigit - 10);
