...
void CVoronoiTessellation::Init (const TArray<CVector2D> &Points, double rWidth, double rHeight)
...
//	Makes sure this is a valid polygon. If not, deletes it.
	...
	//	(And delete the extra ones.)
		...
		for (i = 0; i < Site.Polygon.GetCount(); i++)
			{
			const CVector2D &vP1 = Site.Polygon[i];
			const CVector2D &vP2 = Site.Polygon[(i + 1) % Site.Polygon.GetCount()];
