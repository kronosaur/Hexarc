...
TDatumPropertyHandler<CAEONReanimator3D> CAEONReanimator3D::m_Properties = {
		...
		},
	{
		...
		[](const CAEONReanimator3D &Obj, const CString &sProperty)
			{
			return CDatum(CLuminousScene3D::AsID(Obj.m_Model.GetMode()));
			},
		[](CAEONReanimator3D &Obj, const CString &sProperty, CDatum dValue, CString *retsError)
			{
...
//	Adds an animation for the given property.
	...
	IAnimator3D::Type iAnimationType = IAnimator3D::AsType(dDesc.GetElement(FIELD_TYPE).AsStringView());
	...
	switch (ILuminousObj3D::GetPropertyDesc(iProp).iType)
		...
		case Obj3DPropType::Bool:
			...
			switch (iAnimationType)
				{
				case IAnimator3D::Type::Constant:
...
CDatum CAEONReanimator3D::Create ()
...
//	Creates an empty scene.
	...
	{
	return CDatum(new CAEONReanimator3D);
	}
...
size_t CAEONReanimator3D::OnCalcSerializeSizeAEONScript (CDatum::EFormat iFormat) const
	{
	return 0;
	}
...
void CAEONReanimator3D::OnMarked (void)
...
void CAEONReanimator3D::OnSerialize (CDatum::EFormat iFormat, IByteStream &Stream) const
...
void CAEONReanimator3D::DeserializeAEONExternal (IByteStream& Stream, CAEONSerializedMap &Serialized)
	{
	m_Model = CLuminousScene3D::CreateFromStream(Stream);
	}

void CAEONReanimator3D::SerializeAEONExternal (IByteStream& Stream, CAEONSerializedMap &Serialized) const
	{
	m_Model.Write(Stream);
	}
...
//	}
	...
	for (int i = 0; i < m_Model.GetObjCount(); i++)
		{
		const ILuminousObj3D& Obj = m_Model.GetObj(i);
...
CDatum CAEONReanimator3D::GetAnimation (const ILuminousObj3D& Obj, const IAnimator3D& Animator)
...
//	Returns the animation for the property in an array.
	...
	for (int i = 0; i < Frames.GetCount(); i++)
		...
		dFrame.SetElement(FIELD_TYPE, IAnimator3D::AsID(Frame.iType));
...
//	this object does not need to be drawn, we return Nil.
	...
	{
	TArray<ILuminousObj3D::SPropertyRenderCtx> Props = Obj.GetPropertiesToRender();
	if (Props.GetCount() == 0)
		return CDatum();
...
//	Sets the given property.
	...
	//	Set a constant value
		...
		switch (ILuminousObj3D::GetPropertyDesc(iProp).iType)
			...
			case Obj3DPropType::Vector:
				Obj.SetPropertyVector(iProp, CVector3D(dValue.GetElement(0), dValue.GetElement(1), dValue.GetElement(2)));
				break;
...
const CString& CAEONReanimator3D::StaticGetTypename (void) { return TYPENAME_REANIMATOR; }
