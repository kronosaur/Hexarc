...
class CArchonConsole : public IIOConsole
	{
	public:
		CArchonConsole (const CString &sID = NULL_STR);

		CArchonConsole (const CArchonConsole &Src) = delete;
		CArchonConsole (CArchonConsole &&Src) = delete;
		CArchonConsole &operator= (const CArchonConsole &Src) = delete;
		CArchonConsole &operator= (CArchonConsole &&Src) = delete;

		void AddReaderAccess (const CString &sUsername);
		inline CArchonConsole *AddRef (void) { CSmartLock Lock(m_cs); m_dwRefCount++; return this; }
		void Close (void);
	...
	protected:
		...
		virtual CString OnGetInputLine (void) override { return NULL_STR; }
		virtual bool OnHasInput (void) const override { return false; }
		virtual void OnOutputLine (const CString &sLine) override;
...
class CArchonConsoleManager
	{
	public:
		CArchonConsoleManager (void) { }
		CArchonConsoleManager (const CArchonConsoleManager &Src) = delete;
		CArchonConsoleManager (CArchonConsoleManager &&Src) = delete;

		CArchonConsoleManager &operator= (const CArchonConsoleManager &Src) = delete;
		CArchonConsoleManager &operator= (CArchonConsoleManager &&Src) = delete;

		TSharedPtr<CArchonConsole> Alloc (void);
		void Expire (void);
		bool Find (const CString &sID, TSharedPtr<CArchonConsole> *retpConsole = NULL) const;

	private:
		CCriticalSection m_cs;
		TSortMap<CString, TSharedPtr<CArchonConsole>> m_Consoles;
	};
